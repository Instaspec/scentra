...
export default function RequestPage() {
  ...
  const [standardizedRequest, setStandardizedRequest] = useState<string | null>(
    null
  );
  ...
  const handleSendMessage = async () => {
    ...
    if (!selectedRequestId) {
      ...
      updatedChat = {
        ...
        messages: [
          {
            role: "assistant",
            content: "What kind of fragrance would you like to create?",
          },
    ...
    if (response.ok) {
      ...
      setRequests(
        requests.map((req) =>
          req.id === updatedChat.id ? finalUpdatedChat : req
        )
      );
    ...
    if (selectedRequestId && standardizedRequest) {
      ...
      setRequests((prevRequests) =>
        prevRequests.map((req) =>
          req.id === selectedRequestId ? updatedRequest : req
        )
      );
        ...
        <Sidebar className="border-r">
          ...
          <SidebarContent>
            ...
            <div className="p-4">
              <Button
                className="w-full"
                onClick={sendToFragranceHouse}
                disabled={!standardizedRequest}
              >
                Send Request
              </Button>
            </div>
          ...
          {!selectedRequestId && (
            <div className="flex flex-1 items-center justify-center flex-col gap-4">
              ...
              <div className="text-sm text-muted-foreground text-center space-y-2">
                <p>
                  <strong>Product & Market</strong> – What’s the product type
                  and target audience?
                </p>
                <p>
                  <strong>Fragrance Profile</strong> – Preferred olfactive
                  family, key notes, and reference scents?
                </p>
                <p>
                  <strong>Story & Emotion</strong> – What mood or experience
                  should it evoke?
                </p>
                <p>
                  <strong>Technical Specs</strong> – Desired concentration,
                  ingredient restrictions?
                </p>
                <p>
                  <strong>Market & Positioning</strong> – Competitor references,
                  unique selling points?
                </p>
                <p>
                  <strong>Budget & Timeline</strong> – What are the financial
          ...
          {selectedRequestId && standardizedRequest && (
            <div className="border-b bg-muted/50 p-4">
              <h3 className="mb-2 font-medium">
                Standardized Fragrance Request:
              </h3>
